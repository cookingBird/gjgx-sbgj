<template>
  <div class="mix-table-wrapper">
    <div v-if="configs.switcher" class="mix-table__action-btn">
      <el-button :class="{ 'mix-table__action-btn-item': true, 'selected': type === item }"
        v-for="item in ['混合', '表格', '地图']" :key="item" @click="type = item">
        {{ item }}
      </el-button>
    </div>

    <div v-if="configs.map" v-show="type !== '表格'" class="shadow-content mix-table__map">
      <base-map ref="basemap"></base-map>
    </div>

    <div v-if="configs.table" v-show="type !== '地图'" class="mix-table__table shadow-content">
      <common-table ref="table" @handleCommand="(key, row) => $emit('handleCommand', key, row)"
        @row-click="(row) => $emit('row-click', row)" v-bind="$attrs">
      </common-table>
    </div>
  </div>
</template>
...
.mix-table-wrapper> :not(.mix-table__action-btn) {
  flex-grow: 1;
  flex-basis: 0;
  padding: calc(var(--el-p) *1px) calc(var(--el-p) *1px);
}

.mix-table-wrapper> :not(.mix-table__action-btn):not([hidden])~ :not([hidden]) {
