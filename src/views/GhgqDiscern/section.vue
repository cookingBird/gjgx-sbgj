...
export default {
  ...
  mixins: [
    Refs.createMap("mixMap", "ctx"),
    Refs.createTable("mixTable", "ctx"),
    mapMix(),
  ],
  data() {
    return {
  ...
  computed: {
    ...
    rules() {
      const getValidator =
        (errorMsgs, otherJudge) => (rule, value, callback) => {
          if (!value && value !== 0) {
        ...
        splitMileage: [
          {
            validator: getValidator(
              ...
              (value, callback) => {
                if (
                  value > this.formData.endMileage ||
                  value < this.formData.beginMileage
                ) {
  ...
  async created() {
    ...
    if (this.isEveryStep && Object.prototype.toString.call(this.uniQuery.pipeLineVos) === "[object Array]") {
  ...
  methods: {
    ...
    getSelectedPipeList() {
      return Helper.queryAllSelected({
        ...
        this.pipeList = [
          Object.assign(this.pipeList[0], { children: data.data }),
        ];
        const choosePipe =
          data.data.find((pipe) => pipe.id == this.selectedPipe?.id) ||
          data.data[0];
    ...
    handleDiscern() {
      return Helper.discernOneStep({
        ...
        this.$router.push({
          ...
          query: {
            // ...this.uniQuery,
            backHref: this.uniQuery.backHref,
            taskId: this.taskId,
            taskName: this.taskName,
    ...
    async onSubmit() {
      ...
      try {
        ...
        return Helper.pipeSplitSegment({
          ...
          splitMileage:
            this.actionType === true ? this.formData.splitMileage : null,
...
<style lang="scss" scoped>::v-deep.mix-table-wrapper .mix-table__action {
  top: -54px;
}
